...
export default class Alert {
  ...
  _bindDismiss($alert) {
    $alert.on('click', '.alert-dismiss', (event) => {
  ...
  message(text, type = 'info', dismissable = false) {
    ...
    if (typeof this.options.limit === 'number' && this.$el.find(`.${this.options.classes.base}`).length >= this.options.limit - 1) {
